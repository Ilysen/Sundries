...
{
	[Serializable]
	public class Ava_UnnamedTweaksCollection_Customs_Mindful : BaseSkill
	{
		private static string RecordString => "Ava_UnnamedTweaksCollection_UsedMindful";
		...
		public override bool HandleEvent(GetWaterRitualReputationAmountEvent E)
		{
			...
			{
			CheckFaction:
				if (E.Record.ParentObject.TryGetStringProperty("Ava_UnnamedTweaksCollection_MindfulFaction", out string mindfulOf))
				{
		...
		private void ChooseMindfulFaction(WaterRitualRecord record, bool retroactive = false)
		{
			...
			{
				record.ParentObject.SetStringProperty("Ava_UnnamedTweaksCollection_MindfulFaction", chosenFaction);
				return;
			}
			...
			if (chosenFaction != null)
			{
				if (retroactive)
				{
					The.Game.PlayerReputation.modify(chosenFaction, 25, "Ava_UnnamedTweaksCollection_WaterRitualMindfulAward", Because: $"because you were mindful of their sensitive history with {record.ParentObject.ShortDisplayName}");
				}
				else
					record.ParentObject.SetStringProperty("Ava_UnnamedTweaksCollection_MindfulFaction", chosenFaction);
			}
