...
namespace Ava.UnnamedTweaksCollection.Scripts
{
	public class Helpers
	{
		...
		{
			return Options.GetOption($"Ava_UnnamedTweaksCollection_{tweakType}");
		}
		...
		{
			setting = Options.GetOption($"Ava_UnnamedTweaksCollection_{tweakType}");
			return setting;
		}
		...
		public static bool ShouldBlockFromAutogetAndDisassemble(GameObject obj)
		{
			if (GetTweakSetting(Tweaks.DontTakeYurlsTreeItMakesThemSad, out string setting) == "Never")
				return false;
			if (obj.HasTag("Ava_UnnamedTweaksCollection_NoAutogetInTowns"))
			{
